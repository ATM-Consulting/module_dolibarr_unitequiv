...
$hookmanager->initHooks(array('supplierorderdispatch'));
$parameters=array();
$hookmanager->executeHooks('doActions',$parameters, $commande, $action);
...
if ($action == 'dispatch' && $user->rights->fournisseur->commande->receptionner)
{
	...
	if (! $notrigger && ! $error)
	{
		global $conf, $langs, $user;$parameters=array();
$hookmanager->executeHooks('doActions',$parameters, $commande, $action);
        // Call trigger
				...
				while ($i < $num)
				{
					...
					else
					{
						...
						if ($remaintodispatch || empty($conf->global->SUPPLIER_ORDER_DISABLE_STOCK_DISPATCH_WHEN_TOTAL_REACHED))
						{
							...
							$parameters=array('suffix'=>$suffix);
							$hookmanager->executeHooks('formObjectLineOptions',$parameters, $objp, $action);
